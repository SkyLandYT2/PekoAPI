...
const getUserFollowers = async (req, res) => {
    try {
        ...
        console.log(`Received followers request for userId: ${userId}, page: ${page}`);

        if (!userId || isNaN(userId)) {
            console.error("Invalid or missing userId for followers request");
        ...
        if (!PEKOSECURITY) {
            console.error("Missing PEKOSECURITY for followers request");
        ...
        const apiUrl = `https://www.pekora.zip/apisite/friends/v1/users/${userId}/followers?cursor=${cursor}&limit=100`;
